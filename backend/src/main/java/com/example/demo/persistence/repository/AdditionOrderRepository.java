...
public interface AdditionOrderRepository extends JpaRepository<AdditionOrderPersistence, Long> {
    ...
    @Query("select aop.addition, sum(aop.units) from AdditionOrderPersistence aop where aop.approved = ?3 and aop.time between ?1 and ?2 group by aop.addition order by sum(aop.totalPrice) desc")
