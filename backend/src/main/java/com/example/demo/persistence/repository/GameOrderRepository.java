...
public interface GameOrderRepository extends JpaRepository<GameOrderPersistence, Long> {
   ...
   /* @Query("select gop.game from GameOrderPersistence gop where gop.approved = ?3 and gop.time between ?1 and ?2 group by gop.game order by sum(gop.totalPrice) desc")
